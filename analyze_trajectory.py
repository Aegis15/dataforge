...
def replay_trajectory(
    ...
    for i, action_dict in enumerate(actions):
        ...
        result = (
            obs.action_result.value
            if hasattr(obs.action_result, "value")
            else str(obs.action_result)
        )
    ...
    diagnostics = None
    if obs.grader_diagnostics is not None or hasattr(obs, "grader_diagnostics"):
...
def print_report(analytics: dict[str, Any], verbose: bool = False) -> None:
    ...
    if diag:
        ...
        print(
            f"    Detection Rate:  {formula.get('detection_rate', 0):.4f} (× {formula.get('detection_weight', 0.4)})"
        )
        print(
            f"    Fix Rate:        {formula.get('fix_rate', 0):.4f} (× {formula.get('fix_weight', 0.6)})"
        )
        print(
            f"    FP Penalty:      {formula.get('fp_penalty_total', 0):.4f} ({counts.get('false_positives', 0)} FPs)"
        )
    ...
    if verbose:
        print("\n  Per-Step Detail:")
        print(
            f"  {'Step':>4s}  {'Action':12s}  {'Result':16s}  {'Delta':>8s}  {'Cumul':>8s}  {'Found':>5s}  {'W':>1s}"
        )
