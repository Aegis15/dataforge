...
### Implemented (deployment verification not recorded in this file)
- `playground/api/app.py` — FastAPI backend with `/api/health`, `/api/profile`, `/api/repair`, `/api/samples/{name}` endpoints. Size-cap middleware (1 MB), slowapi rate limiting (10/min/IP, single-worker), CORS for `*.workers.dev`, `*.pages.dev`, and explicit custom origins, advanced-mode gating, ephemeral transaction journals in `TemporaryDirectory`.
...
- `playground/web/DEPLOY.md` — Cloudflare Workers Static Assets deploy runbook.
- `wrangler.toml`, `playground/web/.assetsignore`, `playground/web/_headers`, `scripts/playground/render_web_config.py`, `scripts/playground/verify_frontend_deploy.py` — frontend deploy contract and verification path for Workers Builds.
...
- `DECISIONS.md` — 3 new entries (design-partner gate, Cloudflare+HF hosting, Workers Static Assets deploy alignment).
...
### Post-deploy metrics
- Docker image size: not recorded here (target <= 600 MB)
- Cold-start latency: not recorded here (target <= 30 s)
- Warm `/api/profile` latency: not recorded here (target <= 5 s)
- Lighthouse performance: not recorded here (target >= 90)
- Lighthouse accessibility: not recorded here (target >= 95)
- Live Space URL: not recorded here
- Live frontend URL: not recorded here

### 2026-04-27 deployment alignment
