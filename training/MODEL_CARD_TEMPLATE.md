...
- Compute target: Kaggle free GPU target, one T4 from a T4 x2 runtime.
...
`{release_status}`. Treat `pipeline_complete_no_heldout_gain` as evidence that
training and publishing worked, not as evidence that model quality improved.
