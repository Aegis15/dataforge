...
from models import DataQualityAction, FixType, IssueType
...
def check_obs(obs, step_label):
    ...
    for field in SCORE_FIELDS:
        ...
        if val is not None and not check_strict(val, f"{step_label}.{field}={val}"):
            all_ok = False
    ...
    for field in SCORE_FIELDS:
        if (
            ...
            and not check_strict(data[field], f"{step_label}.json.{field}={data[field]}")
        ):
            all_ok = False
...
def main():
    ...
    print("\n" + "=" * 70)
    if all_pass:
        print("  ALL FIELDS STRICTLY IN (0, 1) — READY TO SUBMIT (0 failures)")
    else:
